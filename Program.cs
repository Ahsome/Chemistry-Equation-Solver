...
namespace ChemistryEquationSolver
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string equation = Console.ReadLine().Replace(" ", String.Empty);
            string[] equationArray = equation.Split('=');
            string[] reactants = equationArray[0].Split('+');
            string[] products = equationArray[1].Split('+');
            ...
            while (true)
            {
                ...
                {
                    Console.Clear();
                    int[] reactantNumbers = ElementNumbers(reactants);
                    int[] productNumbers = ElementNumbers(products);

                    if (reactantNumbers.Where((t, i) => t != productNumbers[i]).Any())
                    {
